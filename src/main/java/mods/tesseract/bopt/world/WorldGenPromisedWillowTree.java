...
public class WorldGenPromisedWillowTree extends WorldGenAbstractTree {
    ...
    public WorldGenPromisedWillowTree(Block wood, Block leaves, int woodMeta, int leavesMeta, int minTreeHeight, int randomTreeHeight) {
        this(wood, leaves, woodMeta, leavesMeta, minTreeHeight, randomTreeHeight, Blocks.vine, -1);
    }

    public WorldGenPromisedWillowTree(Block wood, Block leaves, int woodMeta, int leavesMeta, int minTreeHeight, int randomTreeHeight, Block vineBlock, int vineMeta) {
        super(false);
        this.wood = wood;
        this.leaves = leaves;
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        int l;
        for(l = random.nextInt(this.randomTreeHeight) + this.minTreeHeight; world.getBlock(x, y - 1, z).getMaterial() == Material.water; --y) {
        }
    ...
    private void generateVines(World par1World, int x, int y, int z, int m) {
        if (this.vineMeta != -1) {
            m = this.vineMeta;
        }

        this.setBlockAndNotifyAdequately(par1World, x, y, z, this.vineBlock, m);
        int i1 = 24;

        while(true) {
            --y;
            if (!par1World.getBlock(x, y, z).isAir(par1World, x, y, z) || i1 <= 0) {
                return;
            }
